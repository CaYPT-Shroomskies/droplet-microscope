...
droplet = droplet_params(volume=1, sl_se_constant=0.08)
...
for i in range(1, visualization_radius):
    ...
    current_rad = (
        radius * i / visualization_radius
    )  # Direction at which this ray points
    predicted_position = (current_rad / viewpoint_height) * (
        platform_height + platform_thickness + viewpoint_height
    )
    ...
    intersect_radius = (
        -light_slope
        - numpy.sqrt(
            (light_slope**2) + results.a_curve * 4 * (viewpoint_height - results.height)
        )
    ) / (-2 * results.a_curve)  # [CHECKED]

    # Solve for air-to-water refraction
    intersection_height = (
        results.a_curve * (intersect_radius**2) + results.height
    )  # Solve the equation [CHECKED]

    tangent_angle = numpy.arctan(
        2 * results.a_curve * intersect_radius
    )  # get the slope of the current x position (always negative), then convert it to radians

    normalized_angle = light_angle - tangent_angle

    refracted_angle = (
        numpy.arcsin(numpy.sin(normalized_angle) / index_refraction_water)
        + tangent_angle
    )  # that one guys law (shell???)

    final_position = intersect_radius + (
        numpy.tan(refracted_angle) * intersection_height
    )  # Position when intersecting with glass [CHECKED]

    # Glass-Water
    # print(numpy.arcsin(numpy.sin(refracted_angle)*index_refraction_water/index_refraction_glass)-refracted_angle)

    refracted_angle = numpy.arcsin(
        numpy.sin(refracted_angle) * index_refraction_water / index_refraction_glass
    )
    ...
    )  # Append a ratio value, ratio to straight-line final position on OBJECT PLANE
...
for i in two_map:
    if i > 0:
        focal_len.append(
            ((platform_thickness + platform_height + viewpoint_height) / ((1 / i) + 1))
        )
