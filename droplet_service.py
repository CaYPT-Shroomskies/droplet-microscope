...
arcPrecision = (
    100  # amount of arcs represented in quarter parabola multiplied into area.
)
...
def basic_solve(droplet):
    ...
    sl_se_constant = (
        droplet.sl_se_constant
    )  # Surface energy between solid phase and liquid, measured in J/m^2
    gl_se_constant = (
        droplet.gl_se_constant
    )  # Surface energy between liquid and gas phase, measured in J/m^2
    ...
    def gasPhaseEnergy(a, c):
        ...
        for n in range(1, arcPrecision):
            ...
            area += (math.pi * 2 * segment_length * n) * math.sqrt(
                (segment_length**2)
                + (
                    ((-a * (segment_length * n) ** 2) + c)
                    - ((-a * (segment_length * (n - 1)) ** 2) + c)
                )
                ** 2
            )
        return area * gl_se_constant

    def gravitationalEnergy(
        a, c
    ):  # a and c belong to the standard equation form ax^2 + c. k is the precision value.
        sum = 0
        k = gravitationalPrecision
        for n in range(k):
            sum -= ((math.pi) * (c**3) * (n**2) * (gravity / 100)) / (
                (k**3) * a
            )  # volsume multiplied by height and gravitational constant
        return sum
    ...
    while geometricRepeats < precision:
        ...
        if (
            stepEnergy > lastEnergy
        ):  # If energy is greater than the last step, reverse direction and make step smaller
        ...
        lastEnergy, lastShape = (
            stepEnergy,
            a,
        )  # Update runtime values for next iteration
    ...
    print(
        ...
        + str(int(lastEnergy * 10**sigfigs) / 10**sigfigs)
        + " mJ"
    )
    print("Used " + str(watchdog) + " steps.")
    print(
        "Graph parameters: a = "
        + str(int(graph[-1][1] * 10**sigfigs) / 10**sigfigs)
        + ", c = "
        + str(int(graph[-1][2] * 10**sigfigs) / 10**sigfigs)
    )  # The last values in the graph
