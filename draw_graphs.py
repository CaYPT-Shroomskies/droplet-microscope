...
def drawHeightVolume():
    ...
    for i in range(int((maximum - minimum) / step_size)):
        ...
        y_graph.append(
            module.basic_solve(droplet_params(volume=minimum + step_size * i)).height
        )
...
def drawHeightSurfaceGasEnergy():
    ...
    for i in range(int((maximum - minimum) / step_size)):
        ...
        y_graph.append(
            module.basic_solve(
                droplet_params(volume=volume, gl_se_constant=minimum + step_size * i)
            ).height
        )
...
def drawHeightSurfaceSolid():
    ...
    for i in range(int((maximum - minimum) / step_size)):
        ...
        y_graph.append(
            module.basic_solve(
                droplet_params(volume=volume, sl_se_constant=minimum + step_size * i)
            ).height
        )
...
def draw_droplet():
    ...
    intercepts = abs(
        math.sqrt(-4 * a * c) / (2 * a)
    )  # Quadratic solve for X intercepts
    print(intercepts)
    precision = 30
